...
(function($){
	...
	$(document).ready(function() {
		$('body').prepend('<div id="supersized-loader"><i class="fa fa-spinner fa-spin"></i></div>').prepend('<div id="supersized"></div>');
	});
	...
	$.supersized = function( options ) {
		...
		var extraParameters = '&extras=owner_name,url_z,url_c,url_b,url_h,url_l,url_o,url_k,url_m,o_dims&format=json&nojsoncallback=1';

    	//Determine where to pull images from
    	switch(options.source){

	    	case 1:		//From a Set
	    		var flickrURL =  'https://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=' + options.api_key + '&photoset_id=' + options.set + '&per_page=' + options.total_slides + '&sort=' + sort_order + extraParameters;
	    		break;
	    	case 2:		//From a User
	    		var flickrURL =  'https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + options.api_key + '&user_id=' + options.user + '&per_page=' + options.total_slides + '&sort=' + sort_order + extraParameters;
	    		break;
	    	case 3:		//From a Group
	    		var flickrURL =  'https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + options.api_key + '&group_id=' + options.group + '&per_page=' + options.total_slides + '&sort=' + sort_order + extraParameters;
	    		break;
        case 4:		//From tags
	    		var flickrURL =  'https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + options.api_key + '&tags=' + options.tags + '&per_page=' + options.total_slides + '&sort=' + sort_order + extraParameters;
	    		break;
	    }
			...
			type: 'GET',
  			...
  			success: function(data){
    			...
    			$.each(flickrResults, function(i,item){
    				...
    				if( typeof item['url_' + options.image_size ] === 'undefined' ) {
					...
					var photoURL  = item['url_' + options.image_size ];

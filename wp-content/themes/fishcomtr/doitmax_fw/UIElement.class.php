		...
		if(count($this->boxes) > 0) {
			...
			foreach($this->boxes as $box) {
				...
				if( $box['type'] != "group_open" && $box['type'] != "group_close" ){
					...
					$output .= '<div class="max_control_container">';

					// add the description if type is not an alert
					if($box['type'] != "alert" && @$box['float'] !== true ){
					  $output .= '<div class="max_control_label">';
					  $output .= '<label for="'.$box['id'].'">'.$box['label'].'</label>';
						$output .= '<div class="description ">'.@$box['desc'].'</div>';
						$output .= '</div>';
					}

					$output .= '<div class="max_control">';
          $output .= '<span class="max_style_holder max_style_'.$box['type'].'">';
				}
					...
					switch($box['type']) {
						...
						case 'group_open':
							...
							$output .= '<div id="max_' . $box['id'] . '" class="' . $clearfix . ' max_meta_row ' . $depend_container . 'max_'.$box['type']. $hidden .'">';
							if(isset( $box['float'] ) && $box['float'] === true){
								$output .= '<div class="max_control_container">';
								$output .= '<div class="max_'.$box['type'].'_header">';
								$output .= '<label for="'.$box['id'].'">'.$box['label'].'</label>';
								$output .= '<div class="description max_description">'.$box['desc'].'</div>';
								$output .= '</div>';
							}
						...
						// Group closer
						case "group_close":
							$output .= '</div>'; // end of max_control_container
                ...
                $parent = "";
                $term = get_term_by( 'id', $taxID, GALLERY_TAXONOMY );
                if( !empty($term->parent) ) $parent = get_term( $term->parent, GALLERY_TAXONOMY );
								...
								$output .= empty($parent) ? '' : ' <small class="grey">('. $parent->name .')</small>';
                $output .= '<span class="ui-icon ui-icon-grip-dotted-vertical"></span></li>';
							}
							$output .= '</ul>';
						...
						case 'sidebar_checkbox':
							...
							foreach($box['options'] as $value => $label) {
								...
								$output .= '<span class="holder ' . $active . '">
												...
												<div class="description max_description">' . $label . '</div>
											</span>';
							}
						...
						case 'upload_image':
							...
							foreach( $data as $index => $value ){
								...
								$output .= '<div class="clearfix max_image_row">
			...
			case 'gallery':
              ...
              $output .= '<div class="format-setting type-post_attachments_checkbox type-checkbox">';
              ...
              /* has posts */ $output .= '<input type="hidden" name="' . esc_attr( $box['id'] ) . '" id="' . esc_attr( $box['id'] ) . '" value="' . esc_attr( $data ) . '" class="widefat" />';

              if ( $query->have_posts() ) {

                  $output .= '<ul id="max-gallery-list" class="clearfix max_gallery_list">';
                  ...
                  while ( $query->have_posts() ) {
                    ...
                    $output .= '<li class="attachment">';
                      $output .= '<div class="attachment-preview '. $orientation . '">';
                        $output .= '<div class="thumbnail">';
                          $output .= '<div class="centered">';
                            $output .= wp_get_attachment_image( $query->post->ID, 'medium' );
                          $output .= '</div>';
                        $output .= '</div>';
                      $output .= '</div>';
                    $output .= '</li>';
                  ...
                  $output .= '<div class="button-holder">';
                    $output .= '<a title="Add images" class="button button-primary addgallery" href="#">Edit gallery</a>';
                    $output .= '<a href="#" title="Delete gallery" class="button delete-gallery">Delete gallery</a>';
                  $output .= '</div>';

              } else {
                  $output .= '<ul id="max-gallery-list" class="clearfix"></ul>';
                  $output .= '<div class="button-holder">';
                    $output .= '<a title="Add images" class="button button-primary button-large addgallery" href="#">Add gallery</a>';
                  $output .= '</div>';
               }

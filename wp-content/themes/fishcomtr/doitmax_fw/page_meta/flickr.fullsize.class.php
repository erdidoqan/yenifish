...
class UIElement_FlickrFullsize extends UIElement {
	...
	public function getMetaBox() {
		...
		$this->addDropdown(array(
			...
			"options"  => array(
				'false'    => __('Date added', MAX_SHORTNAME),
		...
		// Flickr Order
		$this->addDropdown(array(
			'id'       => 'max_fullsize_flickr_order',
			'label'    => __('Sort direction', MAX_SHORTNAME),
			"options"  => array(
				'0' => __('Descending', MAX_SHORTNAME),
				'1' => __('Ascending', MAX_SHORTNAME),
			),
			"standard" => '0'
		));
		...
		$this->addDropdown(array(
			...
			'options'    => array(
				...
				'l' => __('Big, 1024px on the longest side', MAX_SHORTNAME),
				'h' => __('Large image, 1600 on longer side', MAX_SHORTNAME),
				'k' => __('Large image, 2048 on longer side', MAX_SHORTNAME),
				'o' => __('Original image, either JPG, GIF or PNG, depending on source format', MAX_SHORTNAME),
			),
			'standard'   => "b",
			'desc'       => __('Choose the size of the images pulled from flickr to show as fullsize background. If a size is not available from Flickr a smaller size is showing instead.', MAX_SHORTNAME)
		));
